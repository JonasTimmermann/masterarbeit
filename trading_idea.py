...
def main():
    ...
    if st.button('Simulation durchführen'):
        ...
        with st.spinner('Bitte warten...'):
            ...
            print(zf)
            #endwerte_sim = [0] * 260
            endwerte_sim = np.zeros(shape=(anzahl_simulationen,2)) # 4096,12
            endwerte_sim_trading = [0] * anzahl_simulationen # 4096,12
            endwerte_sim_buyandhold = [0] * anzahl_simulationen # 4096,12
            ...
            #"""  
                ...
                columns = ('Prozess (bzw. Buy and Hold)', 'Signal', 'Kauf-Möglich?', 'Position', 'Cash-Bestand','Trading-Position'))
                csv_klassisch = df3.to_csv().encode('utf-8')
           
                print(str(process[259]) + " | " + str(golden[259]))
                endwerte_sim[i][0] = process[259] # Buy and Hold
                endwerte_sim[i][1] = golden[259]  # Trading
                endwerte_sim_trading[i] = process[259]
                endwerte_sim_buyandhold[i] = golden[259]  

                betragBH += round(process[259],2)
                betragTS += round(golden[259],2)
        ...
        with st.spinner('Bitte warten...'):
           ...
           # csv = df2.to_excel("output.xlsx")
            ...
            with spalte_1:
                ...
                st.download_button(
                "Ergebnis als CSV/Excel herunterladen",
                csv,
                "file.csv",
                "text/csv",
                key='download-csv'
